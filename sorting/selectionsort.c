...
void PrintInput(const int *arr, int len);
...
void PrintInput(const int *arr, const int len) {
    for (int i=0; i<len; i++) {
        printf("%d ", arr[i]);
    }
}
...
void SelectionSort(int * arr, const int len) {
    int j = 0;
    // Consider index 0 is the one with the smallest element.
    for (int i = 0; i<len-1; i++) {
        // This for loop is for finding ith smallest element,
        // meaning the elemnt that needs to be at index i in sorted array.
        int min = i;
        for (j=i+1; j<len; j++) {
            // Make j in the inner loop equal i+1 and from this till end look for smallest element.
